...
type
  ...
  public
    ...
    constructor create(hOwner: TDomTree; hParent: Pointer; hTag, hAttrTxt: string; hAttr: TDictionary<string, string>; hTypeTag, hText: string);
    destructor destroy; override;
    function FindNode(hNameTag: string; hIndex: integer; hAttrTxt: string; hAnyLevel: Boolean; dListNode: TNodeList): Boolean;
    function FindTagOfIndex(hNameTag: string; hIndex: integer; hAnyLevel: Boolean; dListNode: TNodeList): Boolean;
    function FindXPath(hXPathTxt: string; dListNode: TNodeList; dListValue: TStringList): Boolean;
...
constructor TDomTreeNode.create(hOwner: TDomTree; hParent: Pointer; hTag, hAttrTxt: string; hAttr: TDictionary<string, string>; hTypeTag, hText: string);
begin
...
function TDomTreeNode.FindNode(hNameTag: string; hIndex: integer; hAttrTxt: string; hAnyLevel: Boolean; dListNode: TNodeList): Boolean;
var
  RegEx: TPerlRegEx;
  i, a: integer;
      ...
      if aNode.Child[j].Attributes <> nil then
        if aNode.Child[j].Attributes.ContainsKey(AttrName) then
          if aNode.Child[j].Attributes.TryGetValue(AttrName, aValue) then
            if AttrValue = aValue then
              dListNode.Add(aNode.Child[j]);
      if hAnyLevel then
...
begin
  ...
  try
    ...
    RegEx.RegEx := '([^\s]*?[^\S]*)=([^\S]*".*?"[^\S]*)|' + '([^\s]*?[^\S]*)=([^\S]*#39.*?#39[^\S]*)|' + '([^\s]*?[^\S]*)=([^\S]*[^\s]+[^\S]*)|' + '(autofocus[^\S]*)()|' + '(disabled[^\S]*)()|' + '(selected[^\S]*)()';
    ...
    begin
      ...
      if hNameTag = '' then
      begin
        if FindAttrChildNode(self, RegEx.Groups[i], RegEx.Groups[i + 1]).Count > 0 then
          result := true;
      end
      else
      begin
            ...
            if TagNodeList[a].Attributes <> nil then
              if TagNodeList[a].Attributes.ContainsKey(RegEx.Groups[i]) then
                if TagNodeList[a].Attributes.TryGetValue(RegEx.Groups[i], tValue) then
                  ...
                  if pos(RegEx.Groups[i + 1], tValue) > 0 then
                  begin
    ...
    begin
      ...
      raise Exception.create('Attribute not found: ' + hAttrTxt);
...
function TDomTreeNode.FindTagOfIndex(hNameTag: string; hIndex: integer; hAnyLevel: Boolean; dListNode: TNodeList): Boolean;
  ...
  begin
    ...
    begin
      if hNameTag <> '*' then
      begin
        if ((AnsiUpperCase(aNode.Child[j].Tag) = AnsiUpperCase(hNameTag)) and (aNode.Child[j].TypeTag <> '</%s>')) or ((AnsiUpperCase(aNode.Child[j].Tag) = '') and (AnsiUpperCase(hNameTag) = 'TEXT()') and (aNode.Child[j].Text <> '')) or ((LeftStr(AnsiUpperCase(aNode.Child[j].Tag), 4) = '<!--') and (AnsiUpperCase(hNameTag) = 'COMMENT()')) then
        begin
          Inc(countNode);
          if (countNode = hIndex) or (hIndex = 0) then
            dListNode.Add(aNode.Child[j])
        end;
        if (hAnyLevel) and (aNode.Child.Count > 0) then
      ...
      begin
        ...
        begin
          ...
          if (SubStringOccurences(AnsiUpperCase(aNode.Child[j].Tag) + ',', enumTags, false) = hIndex) or (hIndex = 0) then
            dListNode.Add(aNode.Child[j])
        end;
        if (hAnyLevel) and (aNode.Child.Count > 0) then
          FindChildTagOfIndex(aNode.Child[j]);
...
function TDomTreeNode.FindXPath(hXPathTxt: string; dListNode: TNodeList; dListValue: TStringList): Boolean;
var
  ...
  var
    Prm: PPrmRec;
  begin
    if (Context = '/') and (trim(mTxtElmt) = '') then
      NextAnyLevel := true
    ...
    begin
      ...
      begin
        ...
        if not TryStrToInt(RegExXPathElmt.Groups[2], Prm.ind) then
          Prm.ind := 0;
      ...
      if CurNode.FindNode(PrmXPath[cPrm].TagName, PrmXPath[cPrm].ind, PrmXPath[cPrm].Attr, PrmXPath[cPrm].AnyLevel, cLNode) then
        for i := 0 to cLNode.Count - 1 do
          if cPrm < PrmCount then
            ...
            dListNode.Add(cLNode[i]);
      cLNode.free;
    end;
    if dListNode.Count > 0 then
      result := true
  end;
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        if (PrmXPath[PrmXPath.Count - 1].TagName[1] = '@') then
        begin
        ...
        begin
          ...
          if (AnsiLowerCase(PrmXPath[PrmXPath.Count - 1].TagName) = 'comment()') or (AnsiLowerCase(PrmXPath[PrmXPath.Count - 1].TagName) = 'text()') then
            for i := 0 to dListNode.Count - 1 do
            begin
              if (AnsiLowerCase(PrmXPath[PrmXPath.Count - 1].TagName) = 'text()') then
                dListValue.Add(dListNode[i].Text)
              else
                dListValue.Add(TDomTreeNode(dListNode[i]).Tag);
...
begin
  ...
  for j := 0 to self.Child.Count - 1 do
    if (LeftStr(self.Child[j].Tag, 4) = '<!--') and (self.Child[j].TypeTag = '%s') and (self.Child[j].Text = '') then
    begin
      Inc(countNode);
      if (countNode = hIndex) or (hIndex = 0) then
      begin
...
begin
  ...
  countNode := 0;
  for j := 0 to self.Child.Count - 1 do
    if (self.Child[j].Tag = '') and (self.Child[j].TypeTag = '') and (self.Child[j].Text <> '') then
    begin
      Inc(countNode);
      if (countNode = hIndex) or (hIndex = 0) then
      begin
...
function TDomTreeNode.GetXPath(hRelative: boolean): string;
  ...
  begin
    ...
    begin
      ...
      begin
        if (Node.Tag = TDomTreeNode(Node.Parent).Child[i].Tag) or ((LeftStr(Node.Tag, 4) = '<!--') and (LeftStr(TDomTreeNode(Node.Parent).Child[i].Tag, 4) = '<!--')) then
          inc(CountNode);
  ...
  begin
    ...
    begin
      if (hRelative) and (TDomTreeNode(Node.Parent).GetAttrValue('id') <> '') then
        result := format('//*[@id=%s]', [TDomTreeNode(Node.Parent).GetAttrValue('id')]) + '/' + result
      else
        result := GetParent(Node.Parent) + TDomTreeNode(Node.Parent).Tag + GetCountTag(Node.Parent) + '/' + result
    end
    else
      result := '.' + result;
  end;
...
begin
  ...
  begin
    if LeftStr(self.Tag, 4) = '<!--' then
      result := 'comment()'
    else if self.Tag <> '' then
      result := self.Tag
    else
      result := 'text()';
...
var
      ...
      for i := 1 to RegExTag.GroupCount do
        ...
        begin
          try
            ...
            on E: Exception do
              Owner.fParseErr.Add('Warning: not add Attributtes ' + E.ClassName + ' : ' + E.Message + 'Sourse string: ' + mAttrTxt + ';' + chr(13) + chr(10) + ' attributtes: ' + RegExTag.Groups[i]);
          end;
          break;
        end;
    end;
  ...
  begin
    try
      ...
      begin
        ...
        RegExTag.RegEx := '([^\s]*?[^\S]*)=([^\S]*".*?"[^\S]*)|' + '([^\s]*?[^\S]*)=([^\S]*'#39'.*?'#39'[^\S]*)|' + '([^\s]*?[^\S]*)=([^\S]*[^\s]+[^\S]*)|' + '(allowTransparency[^\S]*)()|' + '(allowfullscreen[^\S]*)()|' + '(novalidate[^\S]*)()|' + '(autofocus[^\S]*)()|' + '(itemscope[^\S]*)()|' + '(disabled[^\S]*)()|' + '(readonly[^\S]*)()|' + '(selected[^\S]*)()|' + '(checked[^\S]*)()|' + '(pubdate[^\S]*)()|' + '(nowrap[^\S]*)()|' + '(hidden[^\S]*)()|' + '(async[^\S]*)()';
        ...
        begin
          ...
          if Length(Trim(CheckAttr)) > 0 then
            Owner.fParseErr.Add('Warning: parsed not all attributes, ' + 'sourse string: ' + mAttrTxt + chr(13) + chr(10) + 'not parsed string: ' + Trim(CheckAttr));
          // ***End Check Parsing Tag Attributes Error************
        end
        else
          Owner.fParseErr.Add('Attributtes not found - ' + 'Sourse string: ' + mAttrTxt);
      end;
    except
      on E: Exception do
        Owner.fParseErr.Add('Attributtes - ' + E.ClassName + ' : ' + E.Message + 'Sourse string: ' + mAttrTxt);
    end;
  end;
  ...
  begin
    try
      ...
      result[3] := ''; // type tag
      if LeftStr(trim(mTxt), 2) = '</' then
        result[3] := '</%s>'  //close
      else if RightStr(trim(mTxt), 2) = '/>' then
        result[3] := '<%s/>'  //selfclose
      else if LeftStr(trim(mTxt), 2) = '<!' then
        result[3] := '%s'
      else if LeftStr(trim(mTxt), 2) = '<?' then
        result[3] := '%s'
      else
      ...
      on E: Exception do
        Owner.fParseErr.Add('Tags - ' + E.ClassName + ' : ' + E.Message + 'Sourse string: ' + mTxt);
    end;
  end;
  ...
  begin
    try
      ...
      on E: Exception do
        Owner.fParseErr.Add('Exception Tags - ' + E.ClassName + ' : ' + E.Message + 'Sourse string: ' + mTxt);
    end;
  end;
  ...
  begin
    ...
    ExceptTag := ',META,LINK,IMG,COL,AREA,BASE,BASEFONT,ISINDEX,BGSOUNDCOMMAND,PARAM,INPUT,EMBED,FRAME,BR,WBR,HR,TRACK,';
    ...
    begin
      ...
      or (AnsiUpperCase(leftstr(mTxtMatch, 10)) = '<PLAINTEXT>') or (AnsiUpperCase(leftstr(mTxtMatch, 5)) = '<XMP>') or (AnsiUpperCase(leftstr(mTxtMatch, 7)) = '<SCRIPT') or (AnsiUpperCase(leftstr(mTxtMatch, 9)) = '<TEXTAREA')
    ...
    begin
      ...
      if tag_txt[2] <> '' then
        ChildTree.Child[ind].Child.Add(TDomTreeNode.create(ChildTree.Owner, ChildTree.Child[ind], '', '', nil, '', tag_txt[2]));
    ...
    begin
      ...
      if (pos(',' + AnsiUpperCase(trim(tag_txt[0])) + ',', ExceptTag) = 0) and (LeftStr(tag_txt[0], 2) <> '<?') and (LeftStr(tag_txt[0], 2) <> '<!') then
...
begin
  ...
  try
    ...
    begin
      ...
      // (<[^>]+>[^<]*)           - all remaining  tags
      // [^<]*                   - text
      RegExException := '(<PLAINTEXT>.*?</PLAINTEXT>[^<]*)|' + '(<title>.*?</title>[^<]*)|' + '(<xmp>.*?</xmp>[^<]*)|' + '(<script.*?</script>[^<]*)|' + '(<textarea.*?</textarea>[^<]*)|' +
//                       '(<pre.*?</pre>[^<]*)|'+
        '(<!--.+?-->[^<]*)|';
